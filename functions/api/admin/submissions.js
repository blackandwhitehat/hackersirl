...
export async function onRequestGet({ request, env }) {
  ...
  const status = url.searchParams.get('status') || 'ready,processing,recording';
  const params = {
    status: `in.(${status})`,
    select: 'id,handle,handle_audio_url,body_audio_url,body_audio_anon_url,anon,anon_voice_id,duration_seconds,transcript,suggested_title,suggested_description,status,created_at',
